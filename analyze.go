	...
	if strings.Contains(comandoSeparado[0], "#") {
		...
		for _, valor := range comandoSeparado {
			...
			if valor == "mkdisk" {
				...
				fmt.Println("Para continuar la ejecución presione la tecla ENTER . . .")
				reader := bufio.NewReader(os.Stdin)
				reader.ReadString('\n')
			}else if valor == "loss" {
...
func analizeRmdisk(comandoSeparado *[]string) {
	...
	*comandoSeparado = (*comandoSeparado)[1:]
	...
	if !drive {
		...
		reader := bufio.NewReader(os.Stdin)
		borrar, err := reader.ReadString('\n')
		if err != nil{
			fmt.Println("No se reconoció la tecla: ", err)
			return
		}
		if borrar == "s\n"{
			_, err = os.Stat("MIA/P1/" + driveletter + ".dsk")
    		if err != nil {
    		    if os.IsNotExist(err) {
    		        fmt.Println("El disco :", driveletter, " no existe.")
    		    }
    		} else {
				os.Remove("MIA/P1/" + driveletter + ".dsk")
				fmt.Println("Se eliminó el disco . . .")
    		}
		}else{
			fmt.Println(borrar)
			fmt.Println("Se ha cancelado la elminación del disco . . .")
		}
	}
}
...
func analizeRep(comandoSeparado *[]string) {
	...
	*comandoSeparado = (*comandoSeparado)[1:]
	...
	if !banderaName {
		...
		if nameValor == "tree" {
			fs.RepTree(idValor, pathValor)
		} else if nameValor == "disk" {
			fs.ReporteDisk(idValor, pathValor)
		} else if nameValor == "sb" {
			fs.ReporteSB(idValor, pathValor)
		} else if nameValor == "mbr"{
			fs.ReporteMbr(idValor,pathValor)
		}
